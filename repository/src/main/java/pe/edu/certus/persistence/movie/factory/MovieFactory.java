...
import pe.edu.certus.persistence.movie.adapters.MovieConverters;
import pe.edu.certus.persistence.movie.dao.MovieDAO;
import pe.edu.certus.persistence.movie.port.jpa.ForManagingMovie;

@Component
public class MovieFactory implements Factory<MovieDAO, MovieDTO> {

    @Autowired
    ForManagingMovie forManagingMovie;

    @Autowired
    MovieConverters forMovieConverters;
    @Override
    public MovieDAO factoryToCreateEntity(MovieDTO movieDTO) {
        MovieDAO movieDAO = forMovieConverters.convertToDao(movieDTO);
    ...
    public void createDefaultEntities() {
        factoryToCreateEntity(MovieDTO.builder()
